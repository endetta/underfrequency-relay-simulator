...
check('transport KOMPAK: tinggi CSS ≤ 44 px', () => {
  const m = src.match(/\.transport\s*\{([^}]*)\}/);
  if (!m) throw new Error('.transport CSS tidak ada');
  const h = m[1].match(/height:(\d+(?:\.\d+)?)px/);
  if (!h) throw new Error('height tak dideklarasi');
  if (parseFloat(h[1]) > 44) throw new Error('transport harus ≤ 44 px, dapat ' + h[1]);
});
