...
| `sim` (fasilitas run) | Satu pintu param → run (§12a): `sim.run()` cache + fingerprint param (deteksi perubahan → recompute + clamp playhead bila run menyusut), `sim.restart()` reset playhead + run segar. Kelas bug "run basi" (M7/M8) mengerucut di sini; handler cukup ubah param → `render()` (yang melewati `sim.run()`). Diuji `tools/sim.test.js`. |
